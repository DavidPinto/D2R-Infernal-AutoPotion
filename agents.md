...
- Version `1.9.0-beta` — **gamepad/controller support** (Keys tab).  When enabled, the app sends XInput gamepad D-pad presses instead of keyboard keys for belt actions.  Maps Q=Left, W=Up, E=Down, R=Right.  Requires XInput-compatible controller.  Controller index configurable (0-3).  UI checkbox with descriptive hint.
