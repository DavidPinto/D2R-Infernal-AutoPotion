...
- Version `1.9.8-beta` — **startup speed + connection flow rework**.  Attach/offset-scan moved off the UI thread (`_try_connect` spawns `_connect_worker`; the multi-second signature scan no longer freezes the window at launch).  A cheap process-name snapshot (~2 s cadence) gates all connect attempts — with no D2R.exe the pill shows "Game not found" and nothing is opened/scanned (old unconditional 3 s retry loop removed).  `GAME_PROCESS_NAMES = ["D2R.exe"]` only (loader/manager exes scrapped).  Top-bar button: Connect (detached) / Reconnect (attached); diagnostics scan connects synchronously on its own thread and reports a missing game distinctly.  Live-verified: window ~1.4 s incl. imports; game absent → pill settles on "Game not found", zero attach attempts.  Test suite: **136 tests green**; compileall + headless UI smoke pass.

- Version `1.9.7-beta` — **merc parity + gamepad merc feed (LT)**.  Gamepad mode: merc actions now hold **LT** while tapping the D-pad (D2R controller feed-merc binding) — previously the bare D-pad made the *player* drink.  Probe-verified: GIP report byte[3] = left trigger (0-255) via XInputGetState; byte[2] is not a trigger.  Watcher: merc_rejuv at its threshold now runs `critical=True` — parity with the player's rejuv line, enabling reach-buried-rejuv (same opt-in gate) + unclassified best-effort; merc_heal stays non-critical (matches player non-critical heal).  Already shared via `_act` (verified): grade-aware picking, waste guard, half-duration/margin gates, cooldowns, managed columns, out-of-stock.  Test suite: **135 tests green** (4 new); compileall + headless UI smoke pass.  Unverified in-game: whether D2R's controller binding actually accepts synthetic LT+D-pad (needs a live feed test with the app elevated).
- Version `1.9.6-beta` — **cleanup & UI polish (no behavior changes)**.  Removed the hidden belt-refill/belt-plan UI stubs from app.py (incl. a per-poll `_refresh_belt_info` call against a never-created widget; refill *logic* + config accessors untouched), rewrote stale comments/docstrings ("smart/plain tier" wording gone; watcher module docstring now describes plan_consume + pre-drink/waste-guard/grade-gate), clarified UI texts (Dashboard help points to the Keys tab, margin hint matches the `duration × margin` gate, "Smart behavior" heading on Triggers, Calibrate "Saved potion codes" distinguished from top-bar profiles, Diagnostics hint rewritten) and added hover tooltips (`ui/widgets.Tooltip`) to the topbar Reconnect/enable/Hotkey controls.  Repo root: one-off `patch_*.py` prototyping scripts removed.  Test suite: **131 tests green**; compileall + headless UI smoke pass.
- Version `1.9.5-beta` (user) — **Predictive Drinking toggle + desperation renamed**.  Triggers checkbox gates the pre-drink/poison logic (`behavior["predictive_drinking"]`, default on; off = drink exactly at threshold sliders); "Desperation Mode" renamed **Reach Buried Rejuv** everywhere (`config.reach_buried_rejuv`; same behavior: at critical HP, drink through a full stack above a rejuv to reach it).
- Version `1.9.4-beta` — **granular monitoring: poison state + drain-slope pre-drink**.  Poison: unit states bitfield read from `statsListEx + 0xAF0` (6 x u32, bit b of word i = state id; poison = state 2) via `GameReader._read_unit_states()` → `PlayerSnapshot.states`/`poisoned`; poisoned puts HP on the heal line so the app drinks before poison (which keeps ticking in town / after fights) hurts.  Pre-drink: `PotionWatcher` keeps a rolling (t, HP, mana) window (`_vitals`) and a per-stat drain slope (`_predict_drop`); a stat that would cross its threshold within the 1.0 s lead (`_PRE_DRINK_LEAD`) is treated as already there, so the restore-over-duration potion is already delivering when the bar empties.  Both flow through the normal `plan_consume` path (cooldowns, waste guard, reach-buried-rejuv, managed columns, out-of-stock all still apply) and stop when the drain slows / state clears.  `_now` is injectable (deterministic tests).  Test suite: **131 tests green** (6 new); compileall + headless UI smoke pass; live snapshot validated against the running game (in-game, states `[105]`, not poisoned).  Remaining: one in-game poison hit to confirm the bit flips; enemy-nearby scan still needs a fight sample.
