...
- Version `1.8.4-beta` — **UI struct calibration framework for reliable menu detection**.  The `ui-v3` signature resolves to a ghost copy; real menu flags are in heap memory.  Added `GameReader.calibrate_ui_struct()` to find the live struct by detecting inventory flag changes (open/close inventory), persisted as `calibrated_ui_address` in config.  `open_menus()` uses calibrated address when available, eliminating false "menus open" from ghost struct.  Calibrate UI tab integration deferred.  Test suite: **118 tests green**; compileall + headless UI smoke pass.
