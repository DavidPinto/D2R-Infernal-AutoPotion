...
- Version `1.8.6-beta` — **full menu calibration: learns struct address + flag indices per build**. Calibrate tab "Calibrate menus" guides user to close all, then open/close each blocking menu (Inventory, Stash, Character, etc.). Detects exact byte index for each menu. Persists address + flag map. Works on ANY build — no hardcoded indices. `open_menus()` uses calibrated flags, accurate per-panel detection. Default `pause_when_menus_open = true`. Diagnostics fixed. Test suite: **118 tests green**; compileall + headless UI smoke pass.
