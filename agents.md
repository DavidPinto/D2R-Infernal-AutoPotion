...
- Version `1.8.6-beta` — **robust menu calibration: learns flag indices by watching actual byte changes**. Calibrate tab "Calibrate menus" guides user through close-all → open/close each core menu (Inventory, Stash, Character, etc.). Detects flag indices by scoring actual byte changes (magnitude + 0→non-zero preference), not assuming 0→non-zero. Persists UI struct address + flag index map. Works on ANY build — no hardcoded indices. `open_menus()` uses calibrated flags for accurate per-panel detection. Default `pause_when_menus_open = true`. Diagnostics fixed (`GameReader` accepts config). Test suite: **118 tests green**; compileall + headless UI smoke pass.
- Version `1.8.5-beta` — **one-click menu calibration + Dashboard always shows menu state + diagnostics fixed**. Calibrate tab: "Calibrate menus" button finds live UI struct at `GameData+0x8` (verified pointer chase), persists `calibrated_ui_address`. Simple one-click, no F8 hovering. Dashboard always shows "menus open" when blocking panels detected (pause setting only controls drinking pause). Diagnostics fixed: `GameReader` accepts optional `config`. Known limitation: menu flag indices shifted for this build (Inventory flag reads 0 when open); flag remapping deferred. `pause_when_menus_open` default true (calibration exists). Test suite: **118 tests green**; compileall + headless UI smoke pass.
